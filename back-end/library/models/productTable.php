    ...
    function getProductList($name = '', $desc = '', $product_line = '', $limit = 0)
    {
        ...
        if (count($data) > 0) {
            foreach ($data as $row) {
                ...
                if ($row['status'] == "1") {
                    array_push($this->data, new Product($row['id'], $row['name'], json_decode($row['description']), $row['inital_price'], $row['selling_price'], $row['quantity'], $arrayFiles, $row['color'], $row['capacity'], $row['status']));
                }
            }
        }
        return $result;
